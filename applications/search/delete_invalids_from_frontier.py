...
from datamodel.search.JekahnHunsingk_datamodel import JekahnHunsingkLink
...
@Deleter(JekahnHunsingkLink)
@Getter(JekahnHunsingkLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(JekahnHunsingkLink)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(JekahnHunsingkLink, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_JekahnHunsingk".format(CrawlerFrame.app_id),
